package poche.fm.potunes.playback;
...
import poche.fm.potunes.utils.QueueHelper;
...
public class QueueManager {
    ...
    public boolean isSameBrowsingCategory(@NonNull String mediaId) {
        ...
        String[] currentBrowserHierarchy = MediaIDHelper.getHierarchy(current.getDescription().getMediaId());

        return Arrays.equals(newBrowseHierarchy, currentBrowserHierarchy);
    }
    ...
    public boolean skipQueuePostion(int amount) {
...
//                    QueueHelper.getPlayingQueue(mediaId, mMusicProvider), mediaId);
    ...
    public void updateMetadata() {
        ...
        // 将歌曲封面图片存在MediaSession中，可以用于显示在锁屏和其他地方
        if (metadata.getDescription().getIconBitmap() == null && metadata.getDescription().getIconUri() != null) {
            ...
            AlbumArtCache.getInstance().fetch(albumUri, new AlbumArtCache.FetchListener() {
                ...
                // if still playing the same music, notify the listeners:
                MediaSessionCompat.QueueItem currentMusic = getCurrentMusic();
                if (currentMusic == null) {
                    return;
                }
